    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-[#1A1A2E] bg-[#08080F]/90 backdrop-blur-md">
      ...
      {open && (
        <div className="md:hidden border-t border-[#1A1A2E] bg-[#08080F] px-6 py-4 flex flex-col gap-4">
          <Link href="/app" onClick={() => setOpen(false)} className="text-sm text-[#6B7280] hover:text-white transition-colors">Dashboard</Link>
          <Link href="/app/stream/new" onClick={() => setOpen(false)} className="text-sm text-[#6B7280] hover:text-white transition-colors">Stream</Link>
          <Link href="/app/pact/new" onClick={() => setOpen(false)} className="text-sm text-[#6B7280] hover:text-white transition-colors">Pact</Link>
          <Link href="/app/send" onClick={() => setOpen(false)} className="text-sm text-[#6B7280] hover:text-white transition-colors">Send</Link>
          <Link href="/sdk" onClick={() => setOpen(false)} className="text-sm text-[#6B7280] hover:text-white transition-colors">SDK</Link>
          <div className="pt-2">
            <ConnectButton />
          </div>
        </div>
